...
export default function RosySidebar() {
  ...
  return (
    <div className="h-full flex flex-col text-zinc-900 p-6">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-zinc-500 leading-relaxed">Ecco cosa posso fare per te oggi.</p>
      </div>
      ...
      <div className="relative p-[1px] rounded-2xl bg-gradient-to-br from-[#00D8FF]/40 via-[#6B5CFF]/20 to-[#D400FF]/40 mb-8 overflow-hidden group">
        ...
        <div className="relative bg-zinc-50/90 backdrop-blur-md p-5 rounded-[15px] border border-zinc-200/50">
          <p className="text-xs text-zinc-700 mb-1 font-medium">Potresti recuperare circa</p>
      ...
      <div>
        ...
        <div className="space-y-3">
          {[
            ...
            { icon: Plus, title: "Riempi slot vuoti", desc: "4 slot disponibili oggi", color: "text-zinc-700" },
            ...
            <button key={i} className="w-full flex items-center gap-4 p-3 bg-white border border-zinc-200/80 hover:border-zinc-300 rounded-xl hover:bg-zinc-100/50 transition-all text-left group">
              <div className={`w-10 h-10 rounded-lg bg-white border border-zinc-200 flex items-center justify-center flex-shrink-0 shadow-inner group-hover:bg-zinc-100/80 transition-colors ${action.color}`}>
                <action.icon size={18} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-zinc-900 truncate">{action.title}</div>
                <div className="text-xs text-zinc-500 truncate">{action.desc}</div>
              </div>
              <ChevronRight size={16} className="text-zinc-400 group-hover:text-zinc-500 flex-shrink-0" />
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Insights */}
      <div className="mt-8 bg-zinc-50/50 border border-zinc-200/50 p-4 rounded-xl">
        <h4 className="text-xs font-semibold text-zinc-800 mb-2">Insights di oggi</h4>
        ...
        <div className="flex items-end gap-1.5 h-10 mt-4 mx-2">
           {[3, 4, 2, 5, 8, 12, 10, 6, 4, 3].map((val, i) => (
             <div key={i} className={`flex-1 rounded-t-sm ${i === 5 || i === 6 ? 'bg-gradient-to-t from-[#6B5CFF] to-[#D400FF]' : 'bg-zinc-100'}`} style={{ height: `${val * 8}%` }}></div>
           ))}
        </div>
      </div>
      ...
      <div className="mt-auto pt-8">
        <div className="relative">
          <input 
            ...
            className="w-full bg-white border border-zinc-200 rounded-xl pl-4 pr-12 py-3 text-sm text-zinc-800 placeholder:text-zinc-400 focus:outline-none focus:border-zinc-300 transition-colors"
          />
